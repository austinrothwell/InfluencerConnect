...
instagram_users2 = open('instagram_users2.txt').read().split('\n')

for username in instagram_users2:

    n_posts = 50   # Total number of posts to scrape (images and videos)
    n_images = 20   # Number of images you want to analyze
    ...
    os.system('instagram-scraper ' + username + ' --maximum ' + str(n_posts) +
              ' --media-metadata --destination /Users/Taylor/PycharmProjects/InfluencerConnect/users/' + username)

    # Directory that user images are saved in
    images_directory = '/Users/Taylor/PycharmProjects/InfluencerConnect/users/' + username + '/'
    ...
    os.rename(images_directory + username + '.json', '/Users/Taylor/PycharmProjects/InfluencerConnect/users' + username + '.json')
